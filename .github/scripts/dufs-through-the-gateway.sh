...
# -k on every call below. The gateway is HTTPS with a certificate the device signed itself,
# and the key in it is what lan-tls.sh checks.
...
curl -fsSk --max-time 30 -T "$dir/sent" "$gateway/files/holiday.bin"
cmp "$dir/sent" "$root/holiday.bin"
curl -fsSk --max-time 30 -o "$dir/got" "$gateway/files/holiday.bin"
...
curl -fsSk --max-time 5 -A "Mozilla/5.0 (X11; Linux x86_64)" -o "$dir/index.html" "$gateway/files/"
...
curl -fsSk --max-time 5 -o /dev/null "$gateway$asset"
...
if curl -fsSk --max-time 5 "http://$lan:5000/" >/dev/null 2>&1; then
