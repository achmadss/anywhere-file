...
func TestHealthReportsOKWhenTheDatabaseAnswers(t *testing.T) {
	...
	newHandler(pool, discard, NewMetrics()).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/healthz", nil))
...
func TestUnknownPathIs404(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	newHandler(pool, discard, NewMetrics()).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/", nil))
