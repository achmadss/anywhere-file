...
func TestAppSyncReplacesTheList(t *testing.T) {
	...
	rec := syncAppsReq(t, h, priv, `{"apps":[{"name":"copyparty","type":"http"},{"name":"jellyfin","type":"http"}]}`, nil)
	...
	for _, name := range []string{"copyparty", "jellyfin"} {
	...
	if got := appNames(t, pool, deviceID); len(got) != 1 || got[0] != "copyparty" {
		t.Errorf("apps = %v, want only copyparty", got)
	}
	...
	if got := appNames(t, pool, deviceID); len(got) != 1 || got[0] != "copyparty" {
		t.Errorf("apps after repeat = %v, want only copyparty", got)
	}
...
func TestAppSyncRefusals(t *testing.T) {
	...
	for name, body := range map[string]string{
		"uppercase":      `{"apps":[{"name":"Copyparty","type":"http"}]}`,
		...
		"bad type":       `{"apps":[{"name":"copyparty","type":"HTTP/1.1"}]}`,
		"duplicate":      `{"apps":[{"name":"copyparty","type":"http"},{"name":"copyparty","type":"ssh"}]}`,
	} {
	...
	if rec := syncAppsReq(t, h, priv, `{"apps":[{"name":"copyparty","type":"http"}]}`, func(*http.Request, []byte) {}); rec.Code != http.StatusUnauthorized {
