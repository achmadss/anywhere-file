...
Runs that share a resource need it split too. Two `hosted/cloud/` issues both drop and recreate the
`public` schema, so each gets its own database on the one running PostgreSQL, and both briefs
say not to touch `docker compose`:

```sh
docker compose -f hosted/cloud/docker-compose.yml up -d
docker compose -f hosted/cloud/docker-compose.yml exec -T postgres \
